...
const modulesOptions = ['Pagos', 'Facturación', 'Reportes', 'Usuarios', 'Inventario']
...
const fakeApiSaveBloqueo = () => {
	return new Promise((resolve, reject) => {
		setTimeout(() => {
			if (newBloqueo.value.nombre.toLowerCase() === 'bloqueo de pagos') {
				reject(new Error('Ya existe un bloqueo con este nombre.'))
			} else {
				console.log('Guardando bloqueo:', newBloqueo.value)
...
const createBloqueo = async () => {
	if (
		...
		toast.error('Error de validación', {
			description: 'Nombre, descripción y al menos un módulo son requeridos.',
		})
		return
	}
	...
	try {
		...
		let message = 'No se pudo crear el bloqueo. Inténtalo de nuevo.'
...
<template>
	<div class="container mx-auto py-6 md:py-10">
		<div class="mb-6">
			<h1 class="text-2xl font-bold md:text-3xl">Crear Nuevo Bloqueo</h1>
		</div>
				...
				<CardContent>
					<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<div class="flex flex-col gap-2">
							...
							<Input
								id="nombre"
								v-model="newBloqueo.nombre"
								placeholder="Ej: Bloqueo de pagos"
								required
							/>
						</div>
						...
						<div class="flex flex-col gap-2 md:col-span-2">
							...
							<Input
								id="descripcion"
								v-model="newBloqueo.descripcion"
						...
						<!-- Módulos a bloquear -->
						<div class="flex flex-col gap-2 md:col-span-2">
							<Label for="modulos"> Módulos a Bloquear </Label>
