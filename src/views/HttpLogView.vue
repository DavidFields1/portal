...
const httpLogs = ref<HttpLogEntry[]>([
  ...
  { id: "log_005", method: "GET", action: "Listar Productos", endpoint: "/api/v1/products", module: "Productos", userId: "guest", status: 200, timestamp: "2024-05-15T11:00:00Z", requestPayload: null, responsePayload: { count: 50 } },
  { id: "log_006", method: "POST", action: "Procesar Pago", endpoint: "/api/v1/payments", module: "Pagos", userId: "user-003", status: 500, timestamp: "2024-05-15T11:05:10Z", requestPayload: { amount: 100 }, responsePayload: { error: "Internal Server Error" } },
...
const methods: (HttpLogEntry["method"] | "Todos")[] = [
  "Todos", "GET", "POST", "PUT", "DELETE", "PATCH",
];
const statusOptions = [
...
const filteredLogs = computed(() => {
  ...
  if (selectedStatus.value !== "all") {
    ...
    logs = logs.filter(
      (log) => log.status >= lowerBound && log.status < upperBound
    );
  }
        ...
        <Card>
                          ...
                          <div class="p-4 space-y-4">
                            ...
                            <div class="relative">
                              <div class="flex justify-between items-center mb-1">
                                ...
                                <Button v-if="log.requestPayload" variant="ghost" size="icon" @click="
                                  copyToClipboard(
                                    log.requestPayload,
                                    'Request'
                                  )
                                  " class="h-6 w-6 p-0.5" aria-label="Copiar Request Payload">
                                  <Copy class="h-3 w-3" />
                                </Button>
                              </div>
                            ...
                            <div class="relative">
                              <div class="flex justify-between items-center mb-1">
                                ...
                                <Button v-if="log.responsePayload" variant="ghost" size="icon" @click="
                                  copyToClipboard(
                                    log.responsePayload,
                                    'Response'
                                  )
                                  " class="h-6 w-6 p-0.5" aria-label="Copiar Response Payload">
                                  <Copy class="h-3 w-3" />
                                </Button>
                              </div>
          ...
          <CardFooter v-if="totalPages > 0" class="flex items-center justify-between p-4">
            <div class="text-sm text-muted-foreground">
              {{ filteredLogs.length }} registros totales. Página
              {{ currentPage }} de {{ totalPages }}
            </div>
            <div class="flex items-center gap-2">
              <Button variant="outline" size="sm" @click="previousPage" :disabled="!canGoPrevious">
                <ChevronLeft class="h-4 w-4" />
                <span class="sr-only">Anterior</span>
              </Button>
              <Button variant="outline" size="sm" @click="nextPage" :disabled="!canGoNext">
