...
const filteredPagos = computed(() => {
	...
	if (searchTerm.value) {
		...
		pagos = pagos.filter((pago) => pago.docContable.toLowerCase().includes(lowerSearch))
	}
...
// Watchers
watch(
	[searchTerm, selectedProveedor, selectedEstatus, startDate, endDate, selectedFechaFiltro],
	() => {
		currentPage.value = 1
	},
)
								...
								<TableBody>
									<template v-if="paginatedPagos.length > 0">
										...
										>
											...
											<TableCell class="py-4 text-center">{{
												pago.facturasAsociadas
											}}</TableCell>
											<TableCell class="py-4 w-28 text-center">{{
												formatDate(pago.fechaDocumento)
											}}</TableCell>
											<TableCell class="py-4 w-28 text-center">{{
												formatDate(pago.fechaCarga)
											}}</TableCell>
										</TableRow>
									</template>
