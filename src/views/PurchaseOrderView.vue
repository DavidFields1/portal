...
const statusProgression: OrdenEstatus[] = [
  "Creada",
  "En Tránsito",
  "Recibida",
  "Facturada",
...
const proveedores = computed(() => {
  const uniqueProveedores = new Set(
    allOrdenes.value.map((orden) => orden.proveedor)
  );
  return Array.from(uniqueProveedores);
});

// Lista de estatus para el filtro
const estatusOptions: OrdenEstatus[] = [
  ...statusProgression,
  consignmentStatus,
];
...
const filteredOrdenes = computed(() => {
  ...
  if (selectedProveedor.value !== "all") {
    ordenes = ordenes.filter(
      (orden) => orden.proveedor === selectedProveedor.value
    );
  }
  if (selectedEstatus.value !== "all") {
    ordenes = ordenes.filter(
      (orden) => orden.estatus === selectedEstatus.value
    );
  }
  if (startDate.value || endDate.value) {
    ordenes = ordenes.filter((orden) => {
      const fecha = new Date(orden.fechaCarga);
      const desde = startDate.value
        ? startDate.value.toDate(getLocalTimeZone())
        : null;
      const hasta = endDate.value
        ? endDate.value.toDate(getLocalTimeZone())
        : null;
      if (desde) {
...
watch(
  [searchTerm, selectedProveedor, selectedEstatus, startDate, endDate],
  () => {
    currentPage.value = 1;
  }
);
...
const getFiltersButtonVariant = () => {
  ...
  else if (!showFilters.value && activeFilterCount.value == 0)
    return "outline";
};
</script>
      ...
      <div class="flex-1">
        ...
        <div v-if="paginatedOrdenes.length > 0" class="grid grid-cols-1 gap-5">
          <Card v-for="orden in paginatedOrdenes" :key="orden.ordenCompra"
            ...
            <CardHeader class="flex flex-row items-start justify-between">
              <div>
                <CardTitle class="text-lg text-primary">
                  <span class="text-sm text-accent-foreground">{{ orden.proveedor }} • {{ orden.sociedad }}</span>
                  <br>
                  {{ orden.ordenCompra }}
                </CardTitle>
            ...
            <CardContent class="grid grid-cols-1 gap-6 md:grid-cols-3 md:items-center">
              ...
              <!-- Importe Total -->
              <div class="flex flex-col items-start justify-center rounded-lg border bg-muted/30 p-4 md:items-end">
              ...
              <div class="flex w-full items-center">
                <template v-for="(status, index) in statusProgression" :key="status">
                  ...
                  <div class="z-10 flex flex-col items-center">
                    <div class="flex h-6 w-6 items-center justify-center rounded-full border-2" :class="getStatusProgress(orden.estatus).currentIndex >= index
                    ...
                    <p class="mt-2 text-center text-xs" :class="getStatusProgress(orden.estatus).currentIndex >= index
        ...
        <div v-else class="flex h-64 items-center justify-center rounded-lg border-2 border-dashed">
          <p class="text-center text-muted-foreground">
            No se encontraron órdenes de compra. <br />Intenta ajustar los
            filtros.
          </p>
        </div>
              ...
              <div class="flex flex-col gap-2">
                ...
                <div class="mt-1 flex flex-col gap-2">
                  ...
                  <Popover>
                    ...
                    <PopoverContent class="w-auto p-0">
                      <Calendar v-model="endDate" :min-value="startDate" :max-value="today(getLocalTimeZone())" />
                    </PopoverContent>
                  </Popover>
                </div>
              </div>
