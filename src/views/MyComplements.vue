...
import { Filter, FileText, Calendar as CalendarIcon } from 'lucide-vue-next'
...
const filteredComplementos = computed(() => {
	...
	if (searchTerm.value) {
		...
		complementos = complementos.filter((c) => c.uuid.toLowerCase().includes(lowerSearch))
	}
				...
				<Card>
							...
							<Table>
								...
								<TableBody>
									<template v-if="paginatedComplementos.length > 0">
										<TableRow
											v-for="c in paginatedComplementos"
											:key="c.uuid"
										...
										>
											...
											<TableCell class="font-mono py-3">{{
												c.uuid
											}}</TableCell>
											...
											<TableCell class="py-4 text-center">
												...
												<Badge
													:variant="getFormaPagoBadgeVariant(c.formaPago)"
												>
													{{ c.formaPago }}
												</Badge>
											</TableCell>
											...
											<TableCell class="w-28 text-center">{{
												formatDate(c.fechaCarga)
											}}</TableCell>
											<TableCell class="w-28 text-center">{{
												formatDate(c.fechaTimbrado)
											}}</TableCell>
										</TableRow>
									</template>
									<template v-else>
					...
					<CardFooter class="flex items-center justify-between border-t px-6 py-4">
						<div class="text-sm text-muted-foreground">
							Mostrando {{ paginatedComplementos.length }} de
							{{ sortedComplementos.length }} complementos.
						</div>
