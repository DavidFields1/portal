...
import NoPOInvoiceUploadStepper from "@/components/layout/NoPOInvoiceUploadStepper.vue";
...
// --- Métodos para archivos (sin cambios) ---
const handlePDFUpload = (event: Event) => {
  const target = event.target as HTMLInputElement;
  const file = target.files?.[0];
  if (file) {
    if (file.size > 10 * 1024 * 1024) { toast.error("El archivo PDF es demasiado grande", { description: "Por favor selecciona un archivo menor a 10MB", }); return; }
    if (file.type !== "application/pdf") { toast.error("Tipo de archivo no válido", { description: "Solo se permite PDF para este campo", }); return; }
    pdfFile.value = file;
    toast.success("Archivo PDF cargado correctamente", { description: `${file.name} está listo para procesar`, });
  }
};
const handleXMLUpload = (event: Event) => {
  ...
  const file = target.files?.[0];
  if (file) {
    if (file.size > 2 * 1024 * 1024) { toast.error("El archivo XML es demasiado grande", { description: "Por favor selecciona un archivo menor a 2MB", }); return; }
    if (file.type !== "text/xml" && file.type !== "application/xml" && !file.name.endsWith(".xml")) { toast.error("Tipo de archivo no válido", { description: "Solo se permite XML para este campo", }); return; }
    xmlFile.value = file;
    toast.success("Archivo XML cargado correctamente", { description: `${file.name} está listo para procesar`, });
  }
};
const removePDF = () => { pdfFile.value = null; const fileInput = document.getElementById("pdf-upload") as HTMLInputElement; if (fileInput) fileInput.value = ""; };
const removeXML = () => { xmlFile.value = null; const fileInput = document.getElementById("xml-upload") as HTMLInputElement; if (fileInput) fileInput.value = ""; };
...
const submitInvoice = async () => {
  ...
  try {
    ...
    toast.success("Factura cargada exitosamente", { description: "La factura fue enviada correctamente.", });
    pdfFile.value = null;
    xmlFile.value = null;
...
// Las props que necesita el stepper son muy simples esta vez
const stepperProps = computed(() => ({
  steps: steps,
  currentStepIndex: currentStepIndex.value,
}));

// Controlamos la visibilidad de la barra lateral con el ciclo de vida de la vista
onMounted(() => {
  layoutStore.showRightSidebar(NoPOInvoiceUploadStepper, stepperProps.value);
});
...
// Actualizamos las props en la barra lateral cada vez que cambien
...
<template>
  ...
  <div class="container mx-auto py-6 md:py-10">
    ...
    <!-- 3. La plantilla ahora solo contiene la columna principal -->
