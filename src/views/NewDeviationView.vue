    ...
    setTimeout(() => {
      if (
        newDesviacion.value.descripcion
          .toLowerCase()
          .includes("gastos menores")
      ) {
        reject(
          new Error(
            "Ya existe una desviación con esa descripción."
          )
        );
      } else {
        console.log(
          "Guardando desviación:",
          newDesviacion.value
        );
...
const createDesviacion = async () => {
  if (
    ...
    toast.error("Error de validación", {
      description:
        "Descripción, moneda y monto (mayor a 0) son obligatorios.",
    });
    return;
  }
        ...
        <CardContent>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="flex flex-col gap-2 md:col-span-2">
              ...
              <Input id="descripcion" v-model="newDesviacion.descripcion" placeholder="Ej: Desviación de gastos menores"
                required />
            </div>
            <div class="flex flex-col gap-2">
              ...
              <Select id="moneda" v-model="newDesviacion.moneda">
                ...
                <SelectContent>
                  <SelectItem v-for="mon in monedaOptions" :key="mon" :value="mon">
                    {{ mon }}
                  </SelectItem>
                </SelectContent>
              </Select>
            ...
            <div class="flex flex-col gap-2">
              ...
              <Input id="monto" type="number" v-model.number="newDesviacion.monto" placeholder="Ej: 1000" required />
            </div>
          </div>
        </CardContent>
      </Card>
