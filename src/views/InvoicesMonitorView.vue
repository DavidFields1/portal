  ...
  <div class="container mx-auto py-6 md:py-10">
    ...
    <div class="space-y-4">
      <div class="relative">
        ...
        <Input v-model="invoiceSearchTerm" placeholder="Buscar por folio o proveedor..." class="pl-10" />
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[75vh] overflow-y-auto p-1">
        <Card v-for="invoice in filteredInvoices" :key="invoice.id"
          class="cursor-pointer transition-all hover:shadow-lg hover:border-primary" @click="goToDetail(invoice.id)">
          <div class="flex justify-between items-stretch">
            <div class="p-4 flex-1">
              ...
              <div class="text-xs space-y-1 text-muted-foreground">
                ...
                <p class="flex items-center gap-2">
                  <CalendarIcon class="h-3 w-3" /> Fecha: {{ df.format(new Date(invoice.fecha)) }}
                </p>
              </div>
            </div>
            <div class="flex flex-col justify-center items-center p-4 w-32 rounded-r-lg">
              ...
              <span class="font-bold text-lg">{{ formatCurrency(invoice.total, invoice.moneda).replace(/[A-Z$]+/g, '')
                }}</span>
            </div>
          </div>
        </Card>
