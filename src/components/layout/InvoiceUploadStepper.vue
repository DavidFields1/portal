...
const props = defineProps({
  ...
  invoiceData: { type: Object as () => { folio: string, moneda: string, importe: number | null, sociedad: string }, required: true },
        ...
        <ol class="space-y-4">
          <li v-for="(step, index) in props.steps" :key="step.id" class="flex items-center text-sm">
            <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full mr-3" :class="{
              'bg-primary text-primary-foreground': index === props.currentStepIndex,
              'bg-green-600 text-white': index < props.currentStepIndex,
              'bg-muted text-muted-foreground border': index > props.currentStepIndex
            }">
    ...
    <CardContent class="mt-4 flex-1 overflow-y-auto">
      ...
      <div v-if="props.currentStepIndex === 1" class="space-y-4">
        ...
        <div v-else class="space-y-4">
          <div class="text-center">
            <div class="text-2xl font-bold text-primary">{{ props.selectedGRs.length }}</div>
      ...
      <div v-if="props.currentStepIndex === 2" class="space-y-6">
        ...
        <div>
          ...
          <div class="border-2 border-dashed border-muted-foreground/25 rounded-lg p-4 text-center mt-1">
            <input type="file" id="pdf-upload-stepper" accept=".pdf" class="sr-only" @change="handlePDFFileChange" />
          ...
          <div v-if="props.pdfFile" class="flex items-center justify-between p-2 bg-muted rounded-lg mt-2 text-sm">
        ...
        <div>
          ...
          <div class="border-2 border-dashed border-muted-foreground/25 rounded-lg p-4 text-center mt-1">
            <input type="file" id="xml-upload-stepper" accept=".xml,text/xml,application/xml" class="sr-only"
          ...
          <div v-if="props.xmlFile" class="flex items-center justify-between p-2 bg-muted rounded-lg mt-2 text-sm">
      ...
      <div v-if="props.currentStepIndex === 3" class="space-y-4">
        ...
        <div class="space-y-3">
          <div class="space-y-1">
            ...
            <Input id="folio" :value="props.invoiceData.folio"
              @input="props.onInvoiceDataChange({ ...props.invoiceData, folio: ($event.target as HTMLInputElement).value })" />
          </div>
          <div class="space-y-1">
            <Label for="moneda">Moneda</Label>
            <Select :model-value="props.invoiceData.moneda"
              @update:modelValue="(val) => props.onInvoiceDataChange({ ...props.invoiceData, moneda: val })">
          ...
          <div class="space-y-1">
            ...
            <Input id="importe" type="number" :value="props.invoiceData.importe"
              @input="props.onInvoiceDataChange({ ...props.invoiceData, importe: parseFloat(($event.target as HTMLInputElement).value) || null })" />
          </div>
          <div class="space-y-1">
            <Label for="sociedad">Sociedad</Label>
            <Input id="sociedad" :value="props.invoiceData.sociedad"
              @input="props.onInvoiceDataChange({ ...props.invoiceData, sociedad: ($event.target as HTMLInputElement).value })" />
          </div>
        </div>
        <div class="flex space-x-2 pt-4">
          <Button variant="outline" size="sm" class="flex-1" @click="props.onPrevStep">← Volver</Button>
      ...
      <div v-if="props.currentStepIndex === 4" class="space-y-4">
        ...
        <div class="space-y-3 text-sm">
          <div class="flex justify-between"><span class="text-muted-foreground">Proveedor:</span><span
              class="font-medium">{{ props.currentSupplierName }}</span></div>
          <div class="flex justify-between"><span class="text-muted-foreground">Entradas:</span><span
              class="font-medium">{{ props.selectedGRs.length }}</span></div>
          <div class="flex justify-between"><span class="text-muted-foreground">Total Seleccionado:</span><span
              class="font-semibold">{{ props.formatCurrency(props.totalSelectedAmount) }}</span></div>
          <Separator />
          <div class="flex justify-between"><span class="text-muted-foreground">Archivo PDF:</span><span
              class="font-medium">{{ props.pdfFile?.name }}</span></div>
          <div class="flex justify-between"><span class="text-muted-foreground">Archivo XML:</span><span
              class="font-medium">{{ props.xmlFile?.name }}</span></div>
          <Separator />
          <div class="flex justify-between"><span class="text-muted-foreground">Folio:</span><span
              class="font-medium">{{ props.invoiceData.folio }}</span></div>
          <div class="flex justify-between"><span class="text-muted-foreground">Moneda:</span><span
              class="font-medium">{{ props.invoiceData.moneda }}</span></div>
          <div class="flex justify-between"><span class="text-muted-foreground">Importe:</span><span
              class="font-semibold">{{ props.formatCurrency(props.invoiceData.importe || 0, props.invoiceData.moneda)
              }}</span></div>
          <div class="flex justify-between"><span class="text-muted-foreground">Sociedad:</span><span
              class="font-medium">{{ props.invoiceData.sociedad }}</span></div>
        </div>
        <div class="flex space-x-2 pt-4">
          <Button variant="outline" size="sm" class="flex-1" @click="props.onPrevStep" :disabled="props.isSubmitting">←
            Volver</Button>
